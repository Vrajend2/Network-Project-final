...
function HomePage() {
  ...
  const fetchTableData = () => {
    ...
    axios
      .get(
        `http://138.197.82.220:3000/api/get-table-data/${selectedYearTable}/${storedUserId}`
      )
      .then((response) => {
  ...
  const fetchUsedCategories = () => {
    axios
      .get("http://138.197.82.220:3000/api/get-all-categories")
      .then((response) => {
  ...
  const handleUsedBudgetSubmit = async (e) => {
    ...
    try {
      await axios.post("http://138.197.82.220:3000/api/enter-used-budget", payload);
  ...
  const fetchLineData = (userId, formattedStartMonth, formattedEndMonth) => {
    axios
      .get(
        `http://138.197.82.220:3000/api/get-line-graph-data-range/${usedselectedCategory}/${startYear}/${formattedStartMonth}/${endYear}/${formattedEndMonth}/${userId}`
      )
      .then((response) => {
  ...
  return (
    <div className="pagecontent" style={{ display: "flex" }}>
      ...
      >
        <ul>
          <li>
            <a href="/userGuidelines" tabIndex="3">
              User Manual
            </a>
          </li>
          <li>
