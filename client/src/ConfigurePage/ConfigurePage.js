...
function ConfigurePage({ updateHomePageData }) {
  ...
  useEffect(() => {
    const fetchAllCategories = async () => {
      try {
        const response = await axios.get('http://138.197.82.220:3000/api/get-all-categories');
  ...
  const fetchDelocatedCategories = (year, month, userId) => {
    axios
      .get(`http://138.197.82.220:3000/api/get-budgets/${year}/${month}/${userId}`)
      .then((response) => {
  ...
  const handleAllocationSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://138.197.82.220:3000/api/configure-budget', budgetData);
  ...
  const handleDeallocationSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://138.197.82.220:3000/api/deallocation-budget', deallocateData);
      console.log(response.data);
  ...
  const months = [
    ...
    'July', 'August', 'September', 'October', 'November', 'December'
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        ...
        <form style={{ width: '60%', padding: '20px', border: '1px solid #ccc', borderRadius: '8px' }} onSubmit={handleAllocationSubmit}>
          ...
          <button type="submit" style={{ backgroundColor: '#ffcd56', color: '#fff', padding: '10px', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>Allocate</button>
        </form>

        {/* Deallocation Form */}
        <form style={{ width: '60%', padding: '20px', border: '1px solid #ccc', borderRadius: '8px' }} onSubmit={handleDeallocationSubmit}>
          <label style={{ display: 'block', marginBottom: '10px' }}>
          ...
          <button type="submit" style={{ backgroundColor: '#ffcd56', color: '#fff', padding: '10px', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>Deallocate</button>
        </form>
      </div>
      </div>
    </div>
